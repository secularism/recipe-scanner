...
- [ ] **READ-01**: 详情页通过真实 recipes 详情接口读取菜谱，不再依赖本地 `findRecipeById`。
- [ ] **READ-02**: 从微信分享 path 打开详情页时，可用 path 中的 id/slug 拉取真实菜谱详情。
- [ ] **READ-03**: 收藏页仍使用本地收藏 id 列表，但展示菜谱卡片时从真实接口读取对应菜谱信息。
- [ ] **READ-04**: 历史页仍使用本地历史记录，但点击历史记录可通过真实接口进入详情页。
- [ ] **READ-05**: 网络失败不会清空或破坏本地收藏、历史和草稿数据。

### Verification

- [ ] **VERIFY-01**: 新增 mapper 和 API client 的单元测试，覆盖成功响应、缺失可选字段和错误响应。
- [ ] **VERIFY-02**: 更新现有 e2e/matcher 测试，使 matcher 仍可用本地测试数据验证排序逻辑。
- [ ] **VERIFY-03**: 提交前通过 miniapp type-check、e2e、matcher 测试和 mp-weixin build。
- [ ] **VERIFY-04**: 文档记录微信小程序 request 合法域名/HTTPS 发布前置条件。
...
| READ-04 | Phase 11 | Pending |
| READ-05 | Phase 11 | Pending |
